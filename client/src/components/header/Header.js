import React from "react";
import InfoWidget from "./InfoWidget";
import Hero from "./Hero";
import quote from "../../data/randomQuote";
import ToggleDarkModeButton from "./ToggleDarkModeButton";
import ToggleSoundButton from "./ToggleSoundButton";
import { playSFXAudio, navLinkClickSFXAudio } from "../../assets/sfx";

function Header({ list, needs, haves, darkMode, setDarkMode }) {
  const toggleDarkModeClick = (e) => {
    e.preventDefault();
    e.stopPropagation();
    setDarkMode((prev) => !prev);
    playSFXAudio(navLinkClickSFXAudio);
  };

  const toggleSoundClick = (e) => {
    e.preventDefault();
    e.stopPropagation();
    playSFXAudio(navLinkClickSFXAudio);
  };

  return (
    <header className={`app-header z-40 sticky bg-inherit w-full ${darkMode?'':'bg-gray-50'}`}>
      <div className="app-header-container mx-auto max-w-screen-xl px-2 py-4 sm:px-3 sm:py-6 sm:pb-9 lg:px-8 md:flex sm:items-center sm:justify-between">
        <Hero title="Grocery Shopping List 🛒" darkMode={darkMode} subtitle={quote} />
        <InfoWidget needs={needs} haves={haves} darkMode={darkMode} setDarkMode={setDarkMode} handleClick={toggleDarkModeClick} />
        <ToggleSoundButton handleClick={toggleSoundClick} darkMode={darkMode} />
        <ToggleDarkModeButton darkMode={darkMode} setDarkMode={setDarkMode} handleClick={toggleDarkModeClick} />
        <div className="clear-both invisible" />
      </div>
    </header>
  );
}

export default React.memo(Header);
