  ...
  const memoizedUpdateBackgroundImage = useCallback(
    (element, keyWord, dark) => {
      ...
      if (element.clientHeight > 10 && element.clientWidth > 10) {
        ...
        const bg = `url("https://gen.pollinations.ai/image/${keyWord.replace(cleanBgSrcRegex, '-').replace(/^-+|-+$/g, '')}?width=${x}&height=${y}&nologo=true&model=${keyWord.includes('a') ? 'flux' : 'zimage'}&key=pk_fjIqPFQbnExLFDfL")`;
