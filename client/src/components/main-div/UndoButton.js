      ...
      <button
        ...
        className={`group absolute -top-4 right-1 text-xl rounded-full p-1.5 ${
          darkMode
            ...
            : " text-violet-700 hover:text-violet-800 border-violet-200 border bg-blue-100 hover:bg-violet-200 focus:outline-none focus:ring-2 focus:ring-violet-400 focus:border-violet-300"
        } transition duration-300 ease-in-out active:scale-90`}
        onClick={()=>handleUndo()}
      >
        <GiReturnArrow className="rotate-180 -scale-x-100 transition-transform group-active:rotate-90 group:hover:-scale-x-75 group-active:-scale-x-75 : " />
      </button>
    </div>
  );
};
